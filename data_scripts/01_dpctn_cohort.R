...
# Define diagnosis date based on earliest diabetes code: exclude those diagnosed between -30 and +90 days of registration
...
## But now have ambiguous pregnancy codes - fine, don't use for classification as not clear which category - will be unclassified if only have these codes and otherwise will use type codes
## Remove for latest type code
...
diabetes_type <- dm_code_counts %>%
  ...
  mutate(diabetes_type=case_when(
    `type 1`==0 & `type 2`==0 & gestational==0 & `gestational history`==0 & malnutrition==0 & mody==0 & `other type not specified`==0 & `other/unspec genetic inc syndromic`==0 & secondary==0 ~ "unspecified",
    
    `type 1`>0 & `type 2`==0 & gestational==0 & `gestational history`==0 & malnutrition==0 & mody==0 & `other type not specified`==0 & `other/unspec genetic inc syndromic`==0 & secondary==0 ~ "type 1",
    
    `type 1`==0 & `type 2`>0 & gestational==0 & `gestational history`==0 & malnutrition==0 & mody==0 & `other type not specified`==0 & `other/unspec genetic inc syndromic`==0 & secondary==0 ~ "type 2",
    
    `type 1`==0 & `type 2`==0 & (gestational>0 | `gestational history`>0) & malnutrition==0 & mody==0 & `other type not specified`==0 & `other/unspec genetic inc syndromic`==0 & secondary==0 ~ "gestational",
    
    `type 1`==0 & `type 2`==0 & gestational==0 & `gestational history`==0 & malnutrition>0 & mody==0 & `other type not specified`==0 & `other/unspec genetic inc syndromic`==0 & secondary==0 ~ "malnutrition",
    
    `type 1`==0 & `type 2`==0 & gestational==0 & `gestational history`==0 & malnutrition==0 & mody>0 & `other type not specified`==0 & `other/unspec genetic inc syndromic`==0 & secondary==0 ~ "mody",
    
    `type 1`==0 & `type 2`==0 & gestational==0 & `gestational history`==0 & malnutrition==0 & mody==0 & `other type not specified`>0 & `other/unspec genetic inc syndromic`>0 & secondary==0 ~ "other type not specified",
    
    `type 1`==0 & `type 2`==0 & gestational==0 & `gestational history`==0 & malnutrition==0 & mody==0 & `other type not specified`==0 & `other/unspec genetic inc syndromic`>0 & secondary==0 ~ "other/unspec genetic inc syndromic",
    
    `type 1`==0 & `type 2`==0 & gestational==0 & `gestational history`==0 & malnutrition==0 & mody==0 & `other type not specified`==0 & `other/unspec genetic inc syndromic`==0 & secondary>0 ~ "secondary"),
...
## Check how many have diabetes codes before DOB only, or with T2 and codes in YOB only
## First - only codes in YOB
dm_codes %>% inner_join(cprd$tables$validDateLookup, by="patid") %>% filter(date>=min_dob) %>% distinct(patid) %>% count()
#1,139,360
#lose 10 with people with codes before DOB only
...
#still 1,139,360, so no-one with T2 and codes in YOB only
...
cohort <- cohort %>% analysis$cached("cohort_interim_8", unique_indexes="patid")
...
#3: converting units only affects 3 results anyway
